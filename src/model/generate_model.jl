...
function generate_operation_model(setup::Dict, inputs::Dict, OPTIMIZER::MOI.OptimizerWithAttributes)
    T = inputs["T"]     # Number of time steps (hours)
    Z = inputs["Z"]     # Number of zones
    ...
    if !isempty(inputs["STOR_ALL"])
        ...
        if inputs["REP_PERIOD"] > 1 && !isempty(inputs["STOR_LONG_DURATION"])
            error("LDES constraints and Benders are not integrated yet")#long_duration_storage!(EP, inputs, setup)
        end
...
function generate_investment_model(setup::Dict, inputs::Dict, OPTIMIZER::MOI.OptimizerWithAttributes)
    T = inputs["T"]     # Number of time steps (hours)
    Z = inputs["Z"]     # Number of zones
